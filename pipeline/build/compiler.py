...
def compile_topic_markdown(
    ...
    if len(claims) <= map_reduce_threshold:
        ...
        for expert_id, expert_claims in by_expert.items():
            claims_json = json.dumps([c.model_dump() for c in expert_claims], indent=2)
            map_prompt = f"Summarise {expert_claims[0].person_name}'s position on {topic_title} in 3-5 bullet points with source citations:\n\n{claims_json}"
            map_resp = client.messages.create(
                model=model,
                max_tokens=1024,
                messages=[{"role": "user", "content": map_prompt}],
            )
            intermediate_blocks.append(f"### {expert_claims[0].person_name}\n{map_resp.content[0].text}")
